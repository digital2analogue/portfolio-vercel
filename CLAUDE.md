...
**Screenshot tolerance is ABSOLUTE (`maxDiffPixels: 200`), not a ratio — changed 2026-09-09, reasoning inline in `playwright.config.ts`.** The old `maxDiffPixelRatio: 0.02` was a fraction of each full-page capture, so the allowance grew with page length: 307,072 px on `/tokens`, 212,070 px on the Capital One case. Nothing a regression could do would exceed that. Two consequences to know about: **(1)** several committed baselines had drifted well inside the old allowance (`/tokens` by 142,558 px, `/about` by 25,509 px) and are stale; **(2)** `--update-snapshots` only rewrites a baseline whose comparison *failed*, so anything the tolerance absorbed was never re-recorded — which is how `/contact`'s baseline came to predate the 2026-08-18 availability-copy change. When a baseline now fails, **look at the diff before regenerating**: the point of the tighter gate is that a failure is information, and blanket-regenerating blesses whatever is currently on screen.
...
**Structurally fixed (2026-09-01):** the npm group now carries `update-types: [minor, patch]`, so majors no longer ride inside the weekly grouped PR at all — each arrives on its own and can be declined without stranding the safe bumps behind it. The `ignore` entries for `typescript` and `eslint` stay: they suppress the two majors already known to be unusable, while `update-types` covers the *next* unknown one. The hand-unpicking pattern above is still the recipe if a group ever does go bad, but it should stop being needed. river-intro shipped a WCAG token fix two weeks late learning the same lesson (its #26/#27); decisioning-table has run this shape since its own typescript-7 incident.
...
**Diagrams fit their column and enlarge on tap (2026-08-18).** A ~1240px canvas cannot be read inline on a phone, and two approaches were tried and rejected before this one. **Shrink-to-fit alone** put the smallest labels at 3.0–3.9px at 390px, measured across all eleven. **Panning inline** (a `min-width` floor on a scrolling `.block-diagram__host`) fixed legibility but cut every diagram off mid-sentence and turned reading the case study into a sideways scrub — the owner rejected it on sight, and it was worse than the problem it solved.

What ships: the diagram **fits** its column, and the whole figure is a zoom trigger — the same bargain an image block already makes. `components/Lightbox.tsx` was extracted out of `CaseBlocks.tsx` so both use one overlay; `variant="diagram"` changes only the fit rule. The overlay shows the committed **2× PNG**, not the inline SVG (an `<img>` pans and pinch-zooms more predictably).

- **The legibility floor moved into the overlay**, where panning is expected and fine. `.lightbox--diagram img` carries `width: 100%; min-width: 900px; max-width: 1240px` — `min-width` beats `max-width` in the cascade, so one block gives both behaviours: capped on a wide screen, floored on a phone.
- The overlay is **opaque**, not the base 90% scrim: once content pans, the page showing through behind it reads as a rendering fault. Same call the iPad prototype's stage made.
- The close button is `position: fixed` for this variant. Absolute positions against the *scroll content*, so it slides off-screen the moment you pan.
- Guarded by `tests/visual/diagram-legibility.spec.ts` across **320 → 1600px** on both diagram routes: every diagram fits its column, every one exposes a keyboard-reachable `role="button"` trigger, the page never scrolls horizontally, and opening the overlay at 390px renders the diagram at ≥900px. Geometry and roles only, no screenshots, so no baseline regen.
...
- **OTKit token source of truth is the Figma file, not the deck.** `OTKit Foundation (Variables Library) v1.0.1`, file key `Z6BsAlsbUFEqdMV5OjqoH4`, collection `Theme`. Button rules, from the owner: *primary buttons always use action tokens; secondary buttons always use `background-default` + `border-default`.* **Hover corrected 2026-08-18: secondary (outline) buttons and table/list row hovers both use `background/hover`, not `background/alt`.** An earlier instruction said `background-alt`; that is superseded — `background/alt` is a resting surface (tracks, chips, filled circles), `background/hover` is the hover state.
  ...
  - **RESOLVED: the deck was right, OTKit's action-hover fails AA, and the demo ships it anyway (2026-08-12 / 2026-08-18).** `action-hover` aliases to the *lighter* brand step, so white `foreground/on-action` on it measures **3.24:1 (teal) / 3.57:1 (red)** — under the 4.5:1 that 1.4.3 requires for the button's own label. **1.4.3 has no state exemption**: its only relevant carve-out is "incidental", which covers *inactive* (disabled) controls; hover and pressed are active states and must clear 4.5:1. (1.4.11 is the SC that treats states specially, and it explicitly does *not* require one state to contrast against another — but it governs the indicator, not the label on it.) So this is a real defect in the OTKit token, not a repo bug. The demos previously shipped `#227795`, a stand-in measuring a safe 5.07:1, which made the gate green by *not* reproducing the system; they now ship the true `#2b9abf`. `action-pressed` `#154a5b` was guessed correctly. Don't "fix" this by inventing a passing colour — the fix belongs upstream, in the token's alias.
  - **RESOLVED: `background/hover` is a literal, not a brand alias.** Light `#f8f8f9`, Dark `#1f2530`, and it is the only background token in the set with **no doc-site description**. It is the hover fill for **secondary (outline) buttons and table/list row hovers** (owner, 2026-08-18) — distinct from `background/alt` `#F1F2F4`, which is a resting surface. The demos now use `--otk-hover` for exactly those two cases; `--otk-surface` stays on tracks, active segments and filled circles.
    - It is a **very faint** state: 1.06:1 against `background/default`, where `background/alt` was 1.12:1. That is the token's own design, not a bug here. WCAG 1.4.11 does not require one *state* to contrast with another, so it is not a conformance failure — but hover should never be the only channel carrying meaning on these controls.
    - Every foreground gets *more* contrast on `#f8f8f9` than on the old `#f1f2f4`, so the move was contrast-safe. The one number to keep in mind: `foreground/alt` `#6f737b` is **4.48:1** on it — still short of 4.5:1, which is why the reservation-detail screen uses `#63666d` for muted text in row states.
...
- **Identity avatars: pick a resting pair, never an interaction step.** A neutral identity circle is `background/alt` + `foreground/default` (`.rd-avatar`). A coloured one is an accent **fill** + its matching `foreground/accent/on-<hue>` — for violet that is `#7f5ce8` + white, 4.57:1 (`.rr-demo-reso__avatar`). What it must never be is a `-hover` or `-pressed` step: those are interaction states, and these circles are static decoration (`.rr-demo-reso__avatar` is `aria-hidden`). Found 2026-08-18 — the avatar was filled `#813992` under a comment claiming "accent-purple-pressed", which was wrong in both directions: wrong role, and **not an OTKit value at all** (real violet-pressed is `#5f45ae`). Resolved violet ramp, Figma-verified: `foreground/accent/violet` `#7f5ce8` (Light) / `#9d82ed` (Dark) · `accent/violet-secondary` `#d5c9f7` · `accent/violet-highlight` `#f1edfc` (Light) / `#1a0a47` (Dark) · `accent/violet-hover` `#9f84ed` · `accent/violet-pressed` `#5f45ae` · `foreground/accent/on-violet` `#ffffff`.
