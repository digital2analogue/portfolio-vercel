...
- **iPad specifics.** The dark chrome is *sampled from the source clip*, not invented (`#0d0e12` rail/sidebar, `#111114` service bar, `#252833` raised step, `#f6f7f8` side panel), so it reads as the same product as the floor-plan demos on `/work/ot-design-system`. `.rdp-device` has a `min-width` and `.rdp` scrolls horizontally: below ~860px the shell keeps its true proportions rather than collapsing four columns into slivers (a 114px "record" column is not evidence of anything). Selected states on the dark panels carry an accent bar as well as the raised fill — the fill alone is 1.31:1 against the panel beside it, which SC 1.4.11 covers as a *state indicator*, and `--rdp-edge` is the 3:1 boundary tone for controls there.
