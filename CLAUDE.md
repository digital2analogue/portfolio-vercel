...
- **iPad specifics.** The shell is modelled on the redesigned iPad BOH screen (`public/projects/images/otreservationsnativeiosandroid.png` is the authoritative comp; OTKit's legacy Figma is `lk1Xd5BlZ44ITvJEr0J9bd`, node `3955:4087`). Its dark values are OTKit's **own dark-mode tokens**, not sampled: `background/default #141A26`, `background/elevation #2D333F`, `border/default #6F737B`, `ash-lighter #D8D9DB`. An earlier pass eyedropped them from a clip and produced a chrome that was plausible and wrong — **pull the tokens, don't sample the pixels** — and a second pass then deleted the left icon rail on the strength of the *legacy* frame, which the redesign still has. Check the comp before removing structure. Composition: rail 40 · service lists 232 · record 374 · actions 192, and `.rdp-device` has `min-width: 848px` so the shell fits the 854px case column exactly at desktop. Date/shift/nav controls are **outlined, not filled**.
  - **The trailing square on every list row is the reservation STATUS**, not the table — it resolves through `stateById()` in `lib/reservationStates.ts`, the same 22-state taxonomy the status-dropdown demo on `/work/ot-design-system` edits, so the two can't drift. The table number rides above it. The record's own status button resolves the same way.
  - `--rdp-focus` is the legacy primary blue (8:1 on the panel); OTKit's action teal is only 3.82:1 there — legal for a non-text indicator but muddy. The selected reservation carries an accent bar as well as its fill: the fill alone is 1.31:1 against the panel beside it, which SC 1.4.11 covers as a *state indicator*. Every status fill clears 3:1 on the panel, but on the selected row the weakest (accent-teal) falls to 1.48:1, so the chip carries a ring that switches to white there.
