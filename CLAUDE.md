...
- Behaviour is shared, not copied: `useSectionSpy` (spy + jump + roving keys) and `useEntrance` are exported from `ReservationDetailDemo` and used by both shells. A second hand-rolled copy is how the two selection strips drifted apart the first time.
...
- **iPad specifics.** The shell is modelled on OTKit's real iPad BOH design (Figma `lk1Xd5BlZ44ITvJEr0J9bd`, node `3955:4087` — `Legacy/iPad/Top Navigation with DM` + `iOS-panel-reservations-list/iPad`). Its dark values are OTKit's **own dark-mode tokens** read from that file, not sampled or invented: `background/default #141A26`, `background/elevation #2D333F`, `border/default #6F737B`, `ash-lighter #D8D9DB`, plus the accent set the lists colour-block with. An earlier pass guessed them from a clip and produced a chrome that was plausible and wrong — **pull the tokens, don't eyedrop the pixels.** Two structural facts that pass came out with: there is **no vertical icon rail** (navigation is the drawer button in the service bar plus the list panel's own dock), and the date/shift/nav controls are **outlined, not filled**. `.rdp-device` has `min-width: 848px` — sized so the shell fits the 854px case column exactly at desktop and only pans on narrow viewports, because collapsing the columns gives a 114px "record" that is not evidence of anything. `--rdp-focus` is the legacy primary blue (8:1 on the panel); OTKit's action teal is only 3.82:1 there — legal for a non-text indicator but muddy. The selected reservation carries an accent bar as well as the raised fill: the fill alone is 1.31:1 against the panel beside it, which SC 1.4.11 covers as a *state indicator*.
