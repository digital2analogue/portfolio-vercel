...
**KNOWN GAP — diagrams are illegible on phones.** `.blocks .block-diagram__host svg` is `width: 100%` with no narrow-width treatment, so a 1240-wide canvas renders at 358px on a 390px viewport (scale 0.29). Measured at 390px: the eight `ds-*` diagrams render their smallest labels at **3.0–3.8px**, the three `ot-reservations-*` at **3.5–3.9px** — all of them unreadable. This is pre-existing and uniform, not specific to any one diagram, so **don't "fix" it by shrinking one canvas** — that just makes that diagram inconsistent while staying unreadable. The real fix is one mobile treatment for the block (horizontal scroll at a legible min-width, or the tap-to-open stage `.block-embed__mobile` already uses), applied to all eleven at once.
