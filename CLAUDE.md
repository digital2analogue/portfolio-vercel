...
- **iPad specifics.** The shell is modelled on the redesigned iPad BOH screen (`public/projects/images/otreservationsnativeiosandroid.png` is the authoritative comp; OTKit's legacy Figma is `lk1Xd5BlZ44ITvJEr0J9bd`, node `3955:4087`). Its dark values are OTKit's **own dark-mode tokens**, not sampled: `background/default #141A26`, `background/elevation #2D333F`, `border/default #6F737B`, `ash-lighter #D8D9DB`. An earlier pass eyedropped them from a clip and produced a chrome that was plausible and wrong — **pull the tokens, don't sample the pixels** — and a second pass then deleted the left icon rail on the strength of the *legacy* frame, which the redesign still has. Check the comp before removing structure. Composition: rail 40 · service lists 232 · record 374 · actions 192. The rail ends at its last section (**no footer group**) and the list panel ends at its last row (**no dock**) — the legacy Figma frame has both, the redesign has neither. `.rdp-device` has `min-width: 848px` so the shell fits the 854px case column exactly at desktop. Date/shift/nav controls are **outlined, not filled**.
  - **The trailing square on every list row is the reservation STATUS**, not the table — it resolves through `stateById()` in `lib/reservationStates.ts`, the same 22-state taxonomy the status-dropdown demo on `/work/ot-design-system` edits, so the two can't drift. The table number rides above it; the record's own status button resolves the same way. It is an **outlined icon button with no fill** — the status is carried by the GLYPH's colour, and it runs the row's height. It also keeps `--rdp-chrome` as its own surface rather than inheriting the row's: on the selected row the weakest tone (accent-teal) would fall to 1.48:1 and the status would be unreadable on exactly the row whose record is open.
